...
#include "cyapicallbacks.h"
    ...
    ***************************************/
    ...
    const LIN_1_SCB_UART_INIT_STRUCT LIN_1_SCB_configUart =
    {
        ...
        (uint8) LIN_1_SCB_UART_RX_BREAK_WIDTH
    };
    ...
    *******************************************************************************/
    void LIN_1_SCB_UartInit(const LIN_1_SCB_UART_INIT_STRUCT *config)
    {
        ...
        else
        {
            ...
            LIN_1_SCB_rxBufferSize  =         config->rxBufferSize;

            /* Set TX direction internal variables */
            LIN_1_SCB_txBuffer      =         config->txBuffer;
            LIN_1_SCB_txDataBits    = (uint8) config->dataBits;
            LIN_1_SCB_txBufferSize  =         config->txBufferSize;
            ...
            LIN_1_SCB_UART_RX_CTRL_REG = LIN_1_SCB_GET_UART_RX_CTRL_MODE(config->stopBits)              |
                                        ...
                                        LIN_1_SCB_GET_UART_RX_CTRL_BREAK_WIDTH(config->breakWidth);
...
void LIN_1_SCB_UartSetRxAddressMask(uint32 addressMask)
{
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: LIN_1_SCB_UartSendBreakBlocking
    ****************************************************************************//**
    *
    ...
    *
    *******************************************************************************/
    void LIN_1_SCB_UartSendBreakBlocking(uint32 breakWidth)
    {
        uint32 txCtrlReg;
        uint32 txIntrReg;

        /* Disable all UART TX interrupt source and clear UART TX Done history */
        txIntrReg = LIN_1_SCB_GetTxInterruptMode();
        LIN_1_SCB_SetTxInterruptMode(0u);
        LIN_1_SCB_ClearTxInterruptSource(LIN_1_SCB_INTR_TX_UART_DONE);

        /* Store TX CTRL configuration */
        txCtrlReg = LIN_1_SCB_TX_CTRL_REG;

        /* Set break width */
        LIN_1_SCB_TX_CTRL_REG = (LIN_1_SCB_TX_CTRL_REG & (uint32) ~LIN_1_SCB_TX_CTRL_DATA_WIDTH_MASK) |
                                        LIN_1_SCB_GET_TX_CTRL_DATA_WIDTH(breakWidth);

        /* Generate break */
        LIN_1_SCB_TX_FIFO_WR_REG = 0u;

        /* Wait for break completion */
        while (0u == (LIN_1_SCB_GetTxInterruptSource() & LIN_1_SCB_INTR_TX_UART_DONE))
        {
        }
        ...
        LIN_1_SCB_TX_CTRL_REG = txCtrlReg;
        LIN_1_SCB_SetTxInterruptMode(txIntrReg);
    }
