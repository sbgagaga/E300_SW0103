...
static uint8 Slave_slReadBuf[Slave_I2C_BTLDR_SIZEOF_READ_BUFFER];

/* Reads from this buffer */
static uint8 Slave_slWriteBuf[Slave_I2C_BTLDR_SIZEOF_WRITE_BUFFER];

/* Flag to release buffer to be read */
static uint32 Slave_applyBuffer;
...
static void Slave_I2CResposeInsert(void);


/*******************************************************************************
* Function Name: Slave_I2CCyBtldrCommStart
****************************************************************************//**
*
...
*
* \globalvars
*  Slave_applyBuffer - the flag to release the buffer with a response
*  to be read by the host.
*
*******************************************************************************/
void Slave_I2CCyBtldrCommStart(void)
{
    Slave_I2CSlaveInitWriteBuf(Slave_slWriteBuf, Slave_I2C_BTLDR_SIZEOF_WRITE_BUFFER);
    Slave_I2CSlaveInitReadBuf (Slave_slReadBuf, 0u);

    Slave_SetCustomInterruptHandler(&Slave_I2CResposeInsert);
    Slave_applyBuffer = 0u;
...
/*******************************************************************************
* Function Name: Slave_I2CCyBtldrCommStop
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: Slave_I2CCyBtldrCommReset
****************************************************************************//**
*
...
*
* \globalvars
*  Slave_slRdBufSize - used to store slave read buffer size.
*  Slave_slRdBufIndex - used to store the current index within the
*  slave read buffer.
...
/*******************************************************************************
* Function Name: Slave_I2CCyBtldrCommRead
****************************************************************************//**
*
...
* \globalvars
*  Slave_slStatus  - used to store current status of I2C slave.
*  Slave_slWriteBuf - used to store received command.
*  Slave_slWrBufIndex - used to store current index within slave
*  write buffer.
*
*******************************************************************************/
cystatus Slave_I2CCyBtldrCommRead(uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        while (0u != timeoutMs)
        {
            ...
            if (0u != (Slave_I2C_SSTAT_WR_CMPLT & Slave_slStatus))
            {
                /* Copy number of written bytes */
                *count = (uint16) Slave_slWrBufIndex;
                ...
                (void) memcpy((void *) pData, (const void *) Slave_slWriteBuf,
                                              Slave_BYTES_TO_COPY(*count, size));

                status = CYRET_SUCCESS;
                break;
            }

            CyDelay(Slave_WAIT_1_MS);
            --timeoutMs;
        }
    }
...
/*******************************************************************************
* Function Name: Slave_I2CCyBtldrCommWrite
****************************************************************************//**
*
...
*
* \globalvars
*  Slave_applyBuffer - the flag to release the buffer with
*  to be read by the host.
*
*******************************************************************************/
cystatus Slave_I2CCyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        *count = size;
        (void) memcpy((void *) Slave_slReadBuf, (const void *) pData, (uint32) size);

        /* Read buffer is ready to be released to host */
        Slave_applyBuffer = (uint32) size;
...
/*******************************************************************************
* Function Name: Slave_I2CResposeInsert
****************************************************************************//**
*
...
static void Slave_I2CResposeInsert(void)
{
    if (Slave_CHECK_INTR_SLAVE_MASKED(Slave_INTR_SLAVE_I2C_ADDR_MATCH))
    {
        if (Slave_CHECK_I2C_STATUS(Slave_I2C_STATUS_S_READ))
        {
            ...
            if (0u != Slave_applyBuffer)
            {
                Slave_slRdBufSize  = Slave_applyBuffer;
                Slave_slRdBufIndex = 0u;
                Slave_applyBuffer  = 0u;
            }
        }
        else
        {
            ...
            if (Slave_slRdBufIndex != Slave_slRdBufSize)
            {
                Slave_slRdBufIndex = Slave_slRdBufSize;
            }
        }
    }
}
