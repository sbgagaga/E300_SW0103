...
volatile uint8 Slave_slStatus;          /* Slave Status */

/* Receive buffer variables */
volatile uint8 * Slave_slWrBufPtr;      /* Pointer to Receive buffer  */
volatile uint32  Slave_slWrBufSize;     /* Slave Receive buffer size  */
volatile uint32  Slave_slWrBufIndex;    /* Slave Receive buffer Index */

/* Transmit buffer variables */
volatile uint8 * Slave_slRdBufPtr;      /* Pointer to Transmit buffer  */
volatile uint32  Slave_slRdBufSize;     /* Slave Transmit buffer size  */
volatile uint32  Slave_slRdBufIndex;    /* Slave Transmit buffer Index */
volatile uint32  Slave_slRdBufIndexTmp; /* Slave Transmit buffer Index Tmp */
volatile uint8   Slave_slOverFlowCount; /* Slave Transmit Overflow counter */

#if (Slave_I2C_CUSTOM_ADDRESS_HANDLER_CONST)

    uint32 (*Slave_customAddressHandler)(void) = NULL;

    /*******************************************************************************
    * Function Name: Slave_SetI2cAddressCustomInterruptHandler
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
    void Slave_SetI2cAddressCustomInterruptHandler(uint32 (*func) (void))
    {
        ...
        Slave_customAddressHandler = func;
    }
#endif /* (Slave_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */


/*******************************************************************************
* Function Name: Slave_I2CSlaveStatus
****************************************************************************//**
*
...
*   - Slave_I2C_SSTAT_RD_CMPLT - Slave read transfer complete. 
...
*   - Slave_I2C_SSTAT_RD_BUSY - Slave read transfer is in progress. 
*     Set when master addresses slave with a read, cleared when 
*    Slave_I2C_SSTAT_RD_CMPLT is set.
*   - Slave_I2C_SSTAT_RD_OVFL - Master attempted to read more bytes
*     than are in buffer. Slave continually returns 0xFF byte in this case.
*   - Slave_I2C_SSTAT_RD_ERR - Slave captured error on the bus during 
*     a read transfer. The sources of error are: misplaced Start or Stop 
*     condition or lost arbitration while slave drives SDA.
*   - Slave_I2C_SSTAT_WR_CMPLT - Slave write transfer complete. 
...
*     successfully.
*   - Slave_I2C_SSTAT_WR_BUSY - Slave write transfer is in progress. 
*     Set when the master addresses the slave with a write, cleared when 
*    Slave_I2C_SSTAT_WR_CMPLT is set.
*   - Slave_I2C_SSTAT_WR_OVFL - Master attempted to write past end of 
*     buffer. Further bytes are ignored.
*   - Slave_I2C_SSTAT_WR_ERR - Slave captured error on the bus during
...
*     Slave_I2C_SSTAT_WR_ERR is set. It is recommended to discard write 
*     buffer content in this case.
*
* \globalvars
*  Slave_slStatus  - used to store current status of I2C slave.
*
*******************************************************************************/
uint32 Slave_I2CSlaveStatus(void)
{
    return((uint32) Slave_slStatus);
}


/*******************************************************************************
* Function Name: Slave_I2CSlaveClearReadStatus
****************************************************************************//**
*
...
*
* \sideeffect
*  This function does not clear Slave_I2C_SSTAT_RD_BUSY.
*
* \globalvars
*  Slave_slStatus  - used to store current status of I2C slave.
*
...
uint32 Slave_I2CSlaveClearReadStatus(void)
{
    ...
    Slave_DisableInt();  /* Lock from interruption */

    /* Mask of transfer complete flag and error status */
    status = ((uint32) Slave_slStatus & Slave_I2C_SSTAT_RD_MASK);
    Slave_slStatus &= (uint8) ~Slave_I2C_SSTAT_RD_CLEAR;

    Slave_EnableInt();   /* Release lock */
...
/*******************************************************************************
* Function Name: Slave_I2CSlaveClearWriteStatus
****************************************************************************//**
*
...
*
* \sideeffect
*  This function does not clear Slave_I2C_SSTAT_WR_BUSY.*
*
* \globalvars
*  Slave_slStatus  - used to store current status of I2C slave.
*
...
uint32 Slave_I2CSlaveClearWriteStatus(void)
{
    ...
    /* Mask of transfer complete flag and Error status */
    status = ((uint32) Slave_slStatus & Slave_I2C_SSTAT_WR_MASK);
    Slave_slStatus &= (uint8) ~Slave_I2C_SSTAT_WR_CLEAR;
...
/*******************************************************************************
* Function Name: Slave_I2CSlaveSetAddress
****************************************************************************//**
*
...
void Slave_I2CSlaveSetAddress(uint32 address)
{
    ...
    matchReg = Slave_RX_MATCH_REG;

    matchReg &= ((uint32) ~Slave_RX_MATCH_ADDR_MASK); /* Clear address bits */
    matchReg |= ((uint32)  Slave_GET_I2C_8BIT_ADDRESS(address));

    Slave_RX_MATCH_REG = matchReg;
}


/*******************************************************************************
* Function Name: Slave_I2CSlaveSetAddressMask
****************************************************************************//**
*
...
void Slave_I2CSlaveSetAddressMask(uint32 addressMask)
{
    ...
    matchReg &= ((uint32) ~Slave_RX_MATCH_MASK_MASK);
    matchReg |= ((uint32) (addressMask << Slave_RX_MATCH_MASK_POS));
...
*
* \globalvars
*  Slave_slRdBufPtr   - used to store pointer to slave read buffer.
*  Slave_slRdBufSize  - used to store salve read buffer size.
*  Slave_slRdBufIndex - used to store current index within slave
*  read buffer.
*
*******************************************************************************/
void Slave_I2CSlaveInitReadBuf(uint8 * rdBuf, uint32 bufSize)
{
    ...
    if(NULL != rdBuf)
    {
        ...
        Slave_slRdBufPtr      = (volatile uint8 *) rdBuf; /* Set buffer pointer  */
        Slave_slRdBufSize     = bufSize;                  /* Set buffer size     */
        Slave_slRdBufIndex    = 0u;                       /* Clear buffer index  */
        Slave_slRdBufIndexTmp = 0u;                       /* Clear buffer index  */
...
/*******************************************************************************
* Function Name: Slave_I2CSlaveInitWriteBuf
****************************************************************************//**
*
*  Sets the buffer pointer and size of the write buffer. This function also 
*  resets the transfer count returned with the 
*  Slave_I2CSlaveGetWriteBufSize() function.
*
...
*
* \globalvars
*  Slave_slWrBufPtr   - used to store pointer to slave write buffer.
*  Slave_slWrBufSize  - used to store salve write buffer size.
*  Slave_slWrBufIndex - used to store current index within slave
*   write buffer.
*
*******************************************************************************/
void Slave_I2CSlaveInitWriteBuf(uint8 * wrBuf, uint32 bufSize)
{
    ...
    if(NULL != wrBuf)
    {
        ...
        Slave_slWrBufPtr   = (volatile uint8 *) wrBuf; /* Set buffer pointer  */
        Slave_slWrBufSize  = bufSize;                  /* Set buffer size     */
        Slave_slWrBufIndex = 0u;                       /* Clear buffer index  */
...
/*******************************************************************************
* Function Name: Slave_I2CSlaveGetReadBufSize
****************************************************************************//**
*
*  Returns the number of bytes read by the I2C master since an
*  Slave_I2CSlaveInitReadBuf() or 
*  Slave_I2CSlaveClearReadBuf() function was executed.
...
*
* \sideeffect
*  The returned value is not valid if Slave_I2C_SSTAT_RD_ERR was 
...
{
    return(Slave_slRdBufIndex);
}


/*******************************************************************************
* Function Name: Slave_I2CSlaveGetWriteBufSize
****************************************************************************//**
*
*  Returns the number of bytes written by the I2C master since an
*  Slave_I2CSlaveInitWriteBuf() or 
*  Slave_I2CSlaveClearWriteBuf() function was executed.
...
*
* \sideeffect
*  The returned value is not valid if Slave_I2C_SSTAT_WR_ERR was 
*  captured by the slave.
*
* \globalvars
*  Slave_slWrBufIndex - used to store current index within slave
...
{
    return(Slave_slWrBufIndex);
}


/*******************************************************************************
* Function Name: Slave_I2CSlaveClearReadBuf
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: Slave_I2CSlaveClearWriteBuf
****************************************************************************//**
*
